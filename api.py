...
import task_queue
from app.agents.prioritizer_agent import prioritize_targets
from app.agents.llm_router import normalize_ollama_url
...
ROADMAP_DATA = [
    {"task": "🐳 Build `docker-compose.goosestrike-full.yml`", "status": "✅ Complete"},
    {"task": "🧠 HackGPT API container (linked to n8n)", "status": "✅ Complete"},
    {"task": "🌐 Local CVE API server", "status": "✅ Complete"},
    {"task": "🧬 Claude + HackGPT fallback system", "status": "✅ Complete"},
    {"task": "🔄 n8n workflow `.json` import", "status": "✅ Complete"},
    {"task": "🎯 Target \"prioritizer\" AI agent", "status": "✅ Complete"},
    {"task": "🧭 SVG architecture diagram", "status": "✅ Complete"},
    {"task": "🖥 Dashboard frontend (Armitage-style)", "status": "✅ Complete"},
...
def _extract_response_payload(response: requests.Response) -> Any:
    """Return JSON (if possible) or a short text preview from a response."""
...
class OllamaTestRequest(BaseModel):
    ...
    prompt: str = "GooseStrike Ollama handshake"
    timeout: int = Field(default=10, ge=1, le=120)
...
class ArmitageNodeOut(BaseModel):
    id: str
    label: str
    type: str


class ArmitageEdgeOut(BaseModel):
    source: str
    target: str
    label: Optional[str] = None


class ArmitageGraphOut(BaseModel):
    nodes: List[ArmitageNodeOut]
    edges: List[ArmitageEdgeOut]
...
def _row_to_workflow(row: sqlite3.Row) -> WorkflowOut:
    data = dict(row)
    return WorkflowOut(
        id=data["id"],
        name=data["name"],
...
@app.post("/n8n/workflows/import", response_model=WorkflowOut, status_code=201)
async def import_n8n_workflow(payload: WorkflowImportIn) -> WorkflowOut:
    with get_db() as conn:
        conn.execute(
            """
            INSERT INTO n8n_workflows(name, workflow_json)
            VALUES(?, ?)
            ON CONFLICT(name) DO UPDATE SET
                workflow_json=excluded.workflow_json,
                imported_at=CURRENT_TIMESTAMP
            """,
            (payload.name, json.dumps(payload.workflow)),
        )
        conn.commit()
        row = conn.execute("SELECT * FROM n8n_workflows WHERE name=?", (payload.name,)).fetchone()
        return _row_to_workflow(row)


@app.get("/n8n/workflows", response_model=List[WorkflowOut])
async def list_n8n_workflows() -> List[WorkflowOut]:
    with get_db() as conn:
        rows = conn.execute(
            "SELECT * FROM n8n_workflows ORDER BY imported_at DESC, id DESC"
        ).fetchall()
        return [_row_to_workflow(row) for row in rows]


@app.post("/integrations/test/n8n", response_model=IntegrationTestResponse)
async def test_n8n_integration(payload: IntegrationTestRequest) -> IntegrationTestResponse:
    url = payload.url or os.getenv("N8N_WEBHOOK_TEST_URL")
    if not url:
        raise HTTPException(status_code=400, detail="Provide a URL or set N8N_WEBHOOK_TEST_URL")
    body = payload.payload or {"ping": "goosestrike"}
    try:
        response = requests.post(url, json=body, timeout=payload.timeout)
        details = _extract_response_payload(response)
        return IntegrationTestResponse(ok=response.ok, status_code=response.status_code, details=details)
    except Exception as exc:  # pragma: no cover - network dependent
        return IntegrationTestResponse(ok=False, status_code=None, error=str(exc))


@app.post("/integrations/test/ollama", response_model=IntegrationTestResponse)
async def test_ollama_integration(payload: OllamaTestRequest) -> IntegrationTestResponse:
    base_url = payload.url or os.getenv("OLLAMA_API_URL") or os.getenv("OLLAMA_BASE_URL")
    if not base_url:
        raise HTTPException(status_code=400, detail="Provide a URL or set OLLAMA_BASE_URL")
    endpoint = normalize_ollama_url(base_url)
    model = payload.model or os.getenv("OLLAMA_MODEL", "llama3")
    body = {"model": model, "prompt": payload.prompt, "stream": False}
    try:
        response = requests.post(endpoint, json=body, timeout=payload.timeout)
        details = _extract_response_payload(response)
        return IntegrationTestResponse(ok=response.ok, status_code=response.status_code, details=details)
    except Exception as exc:  # pragma: no cover - network dependent
        return IntegrationTestResponse(ok=False, status_code=None, error=str(exc))
...
@app.post("/agents/prioritize", response_model=PrioritizerResponse)
async def prioritize(payload: PrioritizerRequest) -> PrioritizerResponse:
    result = prioritize_targets(payload.dict())
...
@app.get("/armitage/data", response_model=ArmitageGraphOut)
async def armitage_data() -> ArmitageGraphOut:
    with get_db() as conn:
        return build_armitage_graph(conn)


@app.get("/mock/armitage-data", response_model=ArmitageGraphOut)
async def mock_armitage_data() -> ArmitageGraphOut:
    return build_mock_armitage_graph()


@app.get("/armitage", response_class=HTMLResponse)
async def armitage_view(request: Request):
    resolved_logo = getattr(app.state, "logo_url", "/static/goose_flag_logo.svg")
    with get_db() as conn:
        graph = build_armitage_graph(conn)
    return templates.TemplateResponse(
        "armitage.html",
        {
            "request": request,
...
def build_armitage_graph(conn: sqlite3.Connection) -> ArmitageGraphOut:
    assets = conn.execute("SELECT * FROM assets ORDER BY ip").fetchall()
    if not assets:
        return build_mock_armitage_graph()
    ...
    for asset in assets:
        ...
        node_map.setdefault(
            asset_node_id,
            ArmitageNodeOut(id=asset_node_id, label=asset["ip"], type="asset"),
        )
        services = conn.execute(
            "SELECT * FROM services WHERE asset_id=? ORDER BY port",
            (asset["id"],),
        ...
        for service in services:
            ...
            label = f"{service['port']}/{service['proto']} {service['product'] or ''}".strip()
            node_map.setdefault(
                service_node_id,
                ArmitageNodeOut(id=service_node_id, label=label, type="service"),
            )
            edges.append(
                ArmitageEdgeOut(
            ...
            for cve_row in cves:
                cve = cve_row["cve_id"]
                cve_node_id = f"cve-{cve}"
                node_map.setdefault(
                    cve_node_id,
                    ArmitageNodeOut(id=cve_node_id, label=cve, type="cve"),
                )
                edges.append(
                    ArmitageEdgeOut(
    ...
    return ArmitageGraphOut(nodes=list(node_map.values()), edges=edges)


def build_mock_armitage_graph() -> ArmitageGraphOut:
    return ArmitageGraphOut(
        nodes=[
            ArmitageNodeOut(id="asset-1", label="10.0.0.21", type="asset"),
            ArmitageNodeOut(id="asset-2", label="10.0.0.45", type="asset"),
            ArmitageNodeOut(id="service-1", label="80/tcp nginx", type="service"),
            ArmitageNodeOut(id="service-2", label="443/tcp envoy", type="service"),
            ArmitageNodeOut(id="service-3", label="5432/tcp postgres", type="service"),
            ArmitageNodeOut(id="cve-CVE-2024-1234", label="CVE-2024-1234", type="cve"),
            ArmitageNodeOut(id="cve-CVE-2023-2454", label="CVE-2023-2454", type="cve"),
        ],
        edges=[
            ArmitageEdgeOut(source="asset-1", target="service-1", label="exposes"),
            ArmitageEdgeOut(source="asset-1", target="service-2", label="exposes"),
            ArmitageEdgeOut(source="asset-2", target="service-3", label="exposes"),
            ArmitageEdgeOut(source="service-2", target="cve-CVE-2024-1234", label="CVE"),
            ArmitageEdgeOut(source="service-3", target="cve-CVE-2023-2454", label="CVE"),
        ],
    )
